...
class Weibo(object):
    def __init__(self, config):
        ...
        self.customised_dir = config['customised_dir'] # 用户自定义保存路径
    ...
    def user_to_csv(self):
        ...
        file_dir = self.customised_dir + os.sep + 'weibo'
        ...
        try:
            ...
            for w in tqdm(self.weibo[wrote_count:], desc='Download progress'):
                ...
                if w.get(key):
                    ...
                    logger.info(u'正在下载第{}条微博中的图片/视频，共{}条'.format(download_count, len(self.weibo)-wrote_count))
                download_count += 1
    ...
    def get_filepath(self, type):
        ...
        try:
            ...
            file_dir = self.customised_dir + os.sep + 'weibo' + os.sep + dir_name
...
class App():
    ...
    def init_window(self):
        self.root.title('抢救你的微博内容')

        Label(self.root, text='微博 ID: ').grid(row=0,column=0)
        self.entry_id = Entry(self.root)
        self.entry_id.grid(row=0, column=1)

        self.cal_frame = LabelFrame(self.root, text='选择抓取起始日期:\n（抓取此日期到今天的微博）')
        self.cal_frame.grid(row=1,column=0, rowspan=2, columnspan=2)
        self.cal = Calendar(self.cal_frame, selectmode='day', year=2021, month=1, day=1, date_pattern='y-mm-dd', firstweekday='sunday')
        self.cal.pack()

        self.frame0 = LabelFrame(self.root, text='选择微博抓取类型')
        self.frame0.grid(row=1, column=2)
        self.filter_var = IntVar()
        self.filter_rbtn1 = Radiobutton(self.frame0, variable=self.filter_var, text="仅原创微博", value=1)
        self.filter_rbtn1.pack()
        self.filter_rbtn2 = Radiobutton(self.frame0, variable=self.filter_var, text="原创+转发微博", value=0)
        self.filter_rbtn2.pack()

        self.frame1 = LabelFrame(self.root, text='下载原创图片')
        self.frame1.grid(row=1, column=3)
        self.original_pic_var = IntVar()
        self.original_pic_rbtn1 = Radiobutton(self.frame1, variable=self.original_pic_var, text="是", value=1)
        self.original_pic_rbtn1.pack()
        self.original_pic_rbtn2 = Radiobutton(self.frame1, variable=self.original_pic_var, text="否", value=0)
        self.original_pic_rbtn2.pack()

        self.frame2 = LabelFrame(self.root, text='下载转发图片')
        self.frame2.grid(row=1, column=4)
        self.retweet_pic_var = IntVar()
        self.retweet_pic_rbtn1 = Radiobutton(self.frame2, variable=self.retweet_pic_var, text="是", value=1)
        self.retweet_pic_rbtn1.pack()
        self.retweet_pic_rbtn2 = Radiobutton(self.frame2, variable=self.retweet_pic_var, text="否", value=0)
        self.retweet_pic_rbtn2.pack()

        self.frame3 = LabelFrame(self.root, text='下载原创视频')
        self.frame3.grid(row=2, column=3)
        self.original_vid_var = IntVar()
        self.original_vid_rbtn1 = Radiobutton(self.frame3, variable=self.original_vid_var, text="是", value=1)
        self.original_vid_rbtn1.pack()
        self.original_vid_rbtn2 = Radiobutton(self.frame3, variable=self.original_vid_var, text="否", value=0)
        self.original_vid_rbtn2.pack()

        self.frame4 = LabelFrame(self.root, text='下载转发视频')
        self.frame4.grid(row=2, column=4)
        self.retweet_vid_var = IntVar()
        self.retweet_vid_rbtn1 = Radiobutton(self.frame4, variable=self.retweet_vid_var, text="是", value=1)
        self.retweet_vid_rbtn1.pack()
        self.retweet_vid_rbtn2 = Radiobutton(self.frame4, variable=self.retweet_vid_var, text="否", value=0)
        self.retweet_vid_rbtn2.pack()

        # Label(self.root, text='保存到: ').grid(row=6,column=0)
        # self.entry_dir = Entry(self.root, width=40)
        # self.entry_dir.grid(row=6, column=1)
        # self.btn_dir = Button(self.root, text='选择保存路径(可不填)', command = self.btn_dir_CallBack)
        # self.btn_dir.grid(row=6, column=2)

        self.log_frame = LabelFrame(self.root, text='输出日志')
        self.log_frame.grid(row=7, column=0, columnspan=5)
        self.st = ScrolledText(self.log_frame)
        self.st.configure(font=tk.font.Font(family="Microsoft YaHei", size=9))
        self.st.pack()
        # st.configure(state='disabled')

        self.btn_run = Button(self.root, text='开始抢救！', command = self.btn_run_CallBack)
        self.btn_run.grid(row=10, column=4)

    def btn_dir_CallBack(self):
    ...
    def my_get_config(self):
        self.config_dict = {
            "user_id_list": [""], # 2107627924, 1774984873
            "filter": 0,
        ...
        self.config_dict['user_id_list'] = [self.entry_id.get()]
        self.config_dict['filter'] = self.filter_var.get()
        self.config_dict['since_date'] = self.cal.get_date()
        self.config_dict['original_pic_download'] = self.original_pic_var.get()
        self.config_dict['retweet_pic_download'] = self.retweet_pic_var.get()
        self.config_dict['original_video_download'] = self.original_vid_var.get()
        self.config_dict['retweet_video_download'] = self.retweet_vid_var.get()
        self.config_dict['result_dir_name'] = 0
        # self.config_dict['customised_dir'] = self.entry_dir.get()
        if self.config_dict['customised_dir'] == '':
            self.config_dict['customised_dir'] = os.getcwd()
        print(self.config_dict)
        return self.config_dict

    def btn_run_CallBack(self):
        try:
            config = self.my_get_config()
            wb = Weibo(config)
            wb.start()  # 爬取微博信息
        except Exception as e:
            logger.exception(e)
...
def main():
    try:
        ...
        app = App(root)
        app.init_window()
        text_handler = TextHandler(app.st)
